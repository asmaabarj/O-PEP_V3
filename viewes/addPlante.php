...
if (isset($_POST['submit'])) {
    ...
    $selectedCategoryName = $_POST['categorySelect'];  // Get the category name from the form

    // Retrieve the Category object based on the selected name
    $serviceCat = new ServiceCategorie();
    
    // Correct method name here
    $selectedCategory = $serviceCat->getNomcategorie($selectedCategoryName);

    $newPlantImage = $_POST['image'];
    $newPlantPrice = $_POST['price'];

    $plant = new Plante($newPlantName, $selectedCategory, $newPlantImage, $newPlantPrice);

    $serviceplant = new ServicePlante();
    $serviceplant->addPlante($plant);
}
?>
...
<header class="header sticky w-[100%] top-0 bg-white shadow-md flex items-center justify-between px-8 py-02 z-50 h-[10vh]	">
    ...
    <?php
    $serviceCat = new ServiceCategorie();
    $categories = $serviceCat->selectcategories();
     foreach ($categories as $categorie) : ?>
<option value="<?php echo $categorie->getNomCategorie() ?>"><?php echo $categorie->getNomCategorie() ?></option>
    <?php endforeach; ?>
</select>
